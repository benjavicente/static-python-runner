...
import type { ICodeContext, IStaticExercise, ITestCaseStateInfo, IRunCodeTestOutput, IFileObj } from "~/lib/types";
...
export default function CodeProvider({ url, ...props }) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      const { id, output, error, time, result } = event.data as IRunCodeTestOutput;
      ...
      switch (result) {
        ...
        setTestCaseResults(results => ({  ...results, [id]: { status, output, error, time}}))
    ...
    return <CodeContext.Provider value={{ testCases, testCaseResults, ready, runCode }} {...props} />
}
